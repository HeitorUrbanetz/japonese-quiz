...
function App() {
  ...
  const handleSubmit = e => {
      ...
      if (input.toLowerCase() === hiragana[current].romanji) {
        ...
        localStorage.setItem('maxStreak', Math.max(streak, maxStreak + 1));
    ...
    <div className="min-h-screen bg-slate-800 text-white text-center">
		...
		<div className='mb-8'>
			<form onSubmit={handleSubmit}>
				<input type="text" value={input} onChange={handleChange} className="block w-24 mx-auto pb-2 bg-transparent border-b-2 border-b-white outline-none text-center text-6xl"/>
			</form>
		</div>
